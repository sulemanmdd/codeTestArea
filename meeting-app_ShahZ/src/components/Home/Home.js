...
class Home extends Component {
  ...
  first() {
      ...
      if (!nickname || !number) {
          ...
          this.setState({first: false, second: true, third: false, nickname, number})
      }
        ...
        return new Promise((resolve, reject) => {
          imagesRef.put(file)
              .then(function (sanpshot) {
                  imagesRef.getDownloadURL().then(function (url1) {
                      return new Promise((resolve, reject) => {
                          imagesRef.put(file2)
                              .then(function (sanpshot) {
                                  imagesRef.getDownloadURL().then(function (url2) {
                                      
                                      return new Promise((resolve, reject) => {
                                          imagesRef.put(file3)
                                              .then(function (sanpshot) {
                                                  imagesRef.getDownloadURL().then(url3 => {

                                                      a.setState({ first: false, second: false, third: true, pic1: url1, pic2: url2, pic3: url3});
  ...
  insertData(){
    const { coffee, juice, cocktail, min1, min2, min3,uid, nickname, number, pic1, pic2, pic3, longitude, latitude } = this.state;
    const db = firebase.firestore();
